...
or forms w.r.t. to a ``Function``.  This is described in more detail in
...
used for example to replace a \texttt{Function} with a fixed value for
...
``Function``.  This functionality can be used for example to linearize
...
Also recall that a ``BasisFunction`` represents any (unknown) basis
