...
.. cpp:class:: BoostGraphOrdering
    ...
    .. cpp:function:: static std::vector<std::size_t> compute_cuthill_mckee(const Graph& graph, bool reverse=false)
    
        Compute re-ordering (map[old] -> new) using Cuthill-McKee algorithm


    .. cpp:function:: static std::vector<std::size_t> compute_cuthill_mckee(const std::set<std::pair<std::size_t, std::size_t> >& edges, std::size_t size, bool reverse=false)
